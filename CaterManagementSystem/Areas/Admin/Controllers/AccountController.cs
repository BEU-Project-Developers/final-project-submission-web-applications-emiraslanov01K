...
using CaterManagementSystem.ViewModels; // Əsas LoginViewModel və RegisterViewModel üçün
...
using Microsoft.AspNetCore.Authorization; // HtmlEncoder üçün
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string? returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null && _passwordService.VerifyPasswordHash(model.Password, user.PasswordHash, user.PasswordSalt))
                {
                    ...
                    if (!isAdmin)
                    {
                        _logger.LogWarning("Login attempt for non-admin user '{UserName}' to admin panel.", model.EmailOrUserName);
                ...
                _logger.LogWarning("Failed admin login attempt for '{UserName}'.", model.EmailOrUserName);
        ...
        public async Task<IActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var defaultRole = await _context.Role.FirstOrDefaultAsync(r => r.Name == "User"); // Və ya "PendingAdmin"
                if (defaultRole == null)
                {
                ...
                using (var transaction = await _context.Database.BeginTransactionAsync())
                {
                    try
                    {
                        ...
                        var callbackUrl = Url.Action("ConfirmEmail", "Account", // Əsas AccountController-dakı ConfirmEmail-i istifadə edə bilər
                                           new { userId = user.Id, token = user.ConfirmationToken, Area = "" }, // Area="" əsas Controller-ə yönləndirir
